...
import { callTool, canon, connectMcp, createChecks, login } from "./support.mjs";
...
async function main() {
  ...
  try {
    ...
    checks.check("one server holds both live MCP sessions", (await fetch("http://127.0.0.1:8796/health").then((r) => r.json())).sessions >= 2);
  } finally {
    await Promise.allSettled([m1.close(), m2.close()]);
  }
  checks.finish();
}
