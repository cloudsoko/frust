...
- **REQ-2.2.1 (Lifecycle Hooks):** Extensions MUST be able to subscribe to document lifecycle events (`before_insert`, `validate`, `on_submit`, `on_cancel`, …). → **satisfied for server scripts** by [[2026-08-01 WO-053 hook vocabulary]]: all four classes subscribable in a manifest, each contract stated — `before_insert`/`validate` may mutate, `on_submit`/`on_cancel` fire pre-commit on the docstatus edge and may **reject** (a refused 0→1 leaves docstatus 0 with ADR-009's lattice EVENT untouched, because the write never happens). ADR-015's composition applies per class (owner-first, un-overridable, per-app-attributed). **Honest remainder:** the vocabulary reaches server scripts, not WIT plugins — the `lifecycle-plugin` world exists and old components are proven to keep loading beside it ([[ADR-006 Plugin Capability Surface]] edge-1, spent for the first time), but no plugin subscribes to the new classes yet. `HookClass::Scheduled` remains declared-and-unwired: WO-007's scheduled work runs kernel `Contrib` handlers, not guest code.
