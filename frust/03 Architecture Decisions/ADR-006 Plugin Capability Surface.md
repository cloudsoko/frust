...
> [!important] Policy REFINED by its first live exercise (WO-053, 2026-08-01): **"additive" has exactly one unit — the WORLD.** Measured, three shapes with predictions stated first: adding a func to an existing interface breaks *every* component including shipped artifacts; adding a new interface to an existing world's exports fails load-link (`no exported instance named 'frust:plugin/lifecycle'`); only a **new world beside the old** is additive — old components load *and run*, receiving only the events they export. Anyone growing the surface grows it as a new world; growing an interface or a world's export list is a breaking change no matter how additive it looks. (WO-019's `plugin`/`route-plugin` split reached this by reasoning; WO-053 measured it, both failure messages in [[2026-08-01 WO-053 hook vocabulary]].)
