...
- **`UPDATE … SET expr WHERE cond` evaluates the SET expression for EVERY row — `WHERE` filters which rows are *written*, not which are *evaluated*** (WO-053: a type-guarded `WHERE` cannot protect a type-specific method call in the `SET`; the v8 migration errored on rows the `WHERE` excluded. Guard inside the expression — inline `IF`/type checks — never via the `WHERE` alone. Found because an unrepresentative probe population hid it, then the A2 upgrade test caught it on real shapes.)
